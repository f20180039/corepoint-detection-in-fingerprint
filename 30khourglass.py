...
def load_data():
    ...
    y = np.array(gt)
    z = np.array(mask)
    X_train, X_test, Y_train, Y_test, Z_train, Z_test = train_test_split(x, y, z, test_size=0.002)
...
ae_output = decoded(
    [Hg_3(Hg_2(Hg_1(encoded(input_img)[0]))), encoded(input_img)[1], encoded(input_img)[2], encoded(input_img)[3]])
...
if args.phase == 'train':
    ...
    for filename in files:
        ...
        f = open(os.path.join(os.path.join(test_folder, "Predictions"), name + ".txt"), 'w+')
